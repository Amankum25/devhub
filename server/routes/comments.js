...
router.get('/post/:postId', optionalAuth, validationRules.withPostId, catchAsync(async (req, res) => {
...
// Create new comment
router.post('/', authenticateToken, validationRules.createComment, catchAsync(async (req, res) => {
...
// Update comment
router.put('/:commentId', authenticateToken, validationRules.updateComment, catchAsync(async (req, res) => {
...
// Like/unlike comment
router.post('/:commentId/like', authenticateToken, validationRules.withCommentId, catchAsync(async (req, res) => {
  const { commentId } = req.params;
  const userId = req.user.id;
  const db = database.getDb();

  // Check if comment exists
  const comment = await db.get('SELECT id FROM comments WHERE id = ? AND status = "approved"', [commentId]);
  if (!comment) {
    throw new AppError('Comment not found', 404, 'COMMENT_NOT_FOUND');
  }
  ...
  const currentLikes = await db.get('SELECT likes FROM comments WHERE id = ?', [commentId]);
...
// Delete comment
router.delete('/:commentId', authenticateToken, validationRules.withCommentId, catchAsync(async (req, res) => {
  const { commentId } = req.params;
  const db = database.getDb();

  // Check if comment exists and user owns it
  const comment = await db.get('SELECT userId FROM comments WHERE id = ?', [commentId]);
...
router.get('/admin/all', requireAdmin, catchAsync(async (req, res) => {
  const page = parseInt(req.query.page) || 1;
  const limit = parseInt(req.query.limit) || 20;
  ...
  if (search) {
    conditions.push('(c.content LIKE ? OR u.firstName LIKE ? OR u.lastName LIKE ?)');
...
router.patch('/admin/:commentId/status', requireAdmin, catchAsync(async (req, res) => {
  const { commentId } = req.params;
