...
router.get('/', [
  ...
  query('sort').optional().isIn(['name', 'created', 'posts', 'followers']).withMessage('Invalid sort option')
...
router.get('/:userId/posts', [
  ...
  if (status !== 'published' && req.user?.id !== parseInt(userId) && !req.user?.isAdmin) {
...
router.get('/admin/all', requireAdmin, [
  ...
  query('sort').optional().isIn(['name', 'email', 'created', 'login']).withMessage('Invalid sort option')
], catchAsync(async (req, res) => {
  checkValidation(req);
  ...
  if (search) {
    conditions.push('(u.firstName LIKE ? OR u.lastName LIKE ? OR u.username LIKE ? OR u.email LIKE ?)');
    const searchTerm = `%${search}%`;
...
router.patch('/admin/:userId/status', requireAdmin, [
  ...
  // Check if user exists
  const user = await db.get('SELECT id, isAdmin FROM users WHERE id = ?', [userId]);
  if (!user) {
    throw new AppError('User not found', 404, 'USER_NOT_FOUND');
  }
