...
router.post('/interact', validationRules.aiInteraction, catchAsync(async (req, res) => {
  ...
  const result = await db.run(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `, [req.user.id, tool, input, output, 'completed', Math.floor(input.length / 4), processingTime]);
