...
router.get('/', optionalAuth, validationRules.paginated, catchAsync(async (req, res) => {
...
router.get('/:identifier', optionalAuth, catchAsync(async (req, res) => {
...
// Create new post
router.post('/', authenticateToken, validationRules.createPost, catchAsync(async (req, res) => {
  const { title, content, excerpt, slug, status, visibility, featuredImage, tags, readTime } = req.body;
  const db = database.getDb();
  ...
  while (true) {
    const existingPost = await db.get('SELECT id FROM posts WHERE slug = ?', [finalSlug]);
...
// Update post
router.put('/:postId', authenticateToken, validationRules.updatePost, catchAsync(async (req, res) => {
  ...
  `, [
    ...
    featuredImage !== undefined ? featuredImage : existingPost.featuredImage,
    JSON.stringify(tags || (existingPost.tags ? JSON.parse(existingPost.tags) : [])),
...
// Like/unlike post
router.post('/:postId/like', authenticateToken, validationRules.withPostId, catchAsync(async (req, res) => {
  const { postId } = req.params;
  const userId = req.user.id;
  const db = database.getDb();

  // Check if post exists
  const post = await db.get('SELECT id FROM posts WHERE id = ? AND status = "published"', [postId]);
  if (!post) {
    throw new AppError('Post not found', 404, 'POST_NOT_FOUND');
  }
  ...
  if (existingLike) {
    ...
    // Like
    await db.run('INSERT INTO post_likes (postId, userId) VALUES (?, ?)', [postId, userId]);
    await db.run('UPDATE posts SET likes = likes + 1 WHERE id = ?', [postId]);
...
// Delete post
router.delete('/:postId', authenticateToken, validationRules.withPostId, catchAsync(async (req, res) => {
  const { postId } = req.params;
  const db = database.getDb();

  // Check if post exists and user owns it
  const post = await db.get('SELECT userId FROM posts WHERE id = ?', [postId]);
...
// Get popular tags
router.get('/tags/popular', catchAsync(async (req, res) => {
  const db = database.getDb();
