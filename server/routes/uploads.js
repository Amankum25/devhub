...
const fileFilter = (req, file, cb) => {
  ...
  if (fieldAllowedTypes.includes(file.mimetype)) {
    ...
    cb(new AppError(`Invalid file type for ${file.fieldname}. Allowed types: ${fieldAllowedTypes.join(', ')}`, 400, 'INVALID_FILE_TYPE'), false);
  }
};
...
// Upload avatar
router.post('/avatar', upload.single('avatars'), catchAsync(async (req, res) => {
  if (!req.file) {
...
router.post('/images', upload.array('images', 5), catchAsync(async (req, res) => {
...
// Upload documents
router.post('/documents', upload.array('documents', 3), catchAsync(async (req, res) => {
  if (!req.files || req.files.length === 0) {
    throw new AppError('No files uploaded', 400, 'NO_FILES');
  }
...
// Get file info
router.get('/info/:type/:filename', catchAsync(async (req, res) => {
...
// Delete file
router.delete('/:type/:filename', catchAsync(async (req, res) => {
  const { type, filename } = req.params;
  const filePath = path.join(uploadsDir, type, filename);
