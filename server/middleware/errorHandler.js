...
const errorHandler = (err, req, res, next) => {
  ...
  if (process.env.NODE_ENV === 'development') {
    ...
    if (error.code === 'SQLITE_CONSTRAINT_UNIQUE') {
      error = new AppError('Duplicate value for unique field', 400, 'DUPLICATE_VALUE');
    }
    if (error.code === 'SQLITE_CONSTRAINT_FOREIGNKEY') {
      error = new AppError('Invalid reference to related data', 400, 'FOREIGN_KEY_ERROR');
    }
...
const createValidationError = (message, field = null) => {
  return new AppError(message, 400, field ? `VALIDATION_${field.toUpperCase()}` : 'VALIDATION_ERROR');
};
...
const handleDatabaseError = (error, operation = 'database operation') => {
  ...
  if (error.code === 'SQLITE_CONSTRAINT_FOREIGNKEY') {
    return new AppError('Invalid reference to related data', 400, 'FOREIGN_KEY_ERROR');
  }
