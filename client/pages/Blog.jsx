...
export default function Blog() {
  ...
  const mockPosts = [
    ...
    },
    {
      ...
      excerpt:
        "Learn containerization basics with Docker. From creating your first container to orchestrating multi-service applications, this guide covers it all.",
      author: "Alex Turner",
      authorAvatar: "👨‍🎓",
  ...
  const filterOptions = [
    { value: "all", label: "All Posts", count: mockPosts.length },
    {
      value: "popular",
      label: "Popular",
      count: mockPosts.filter((p) => p.likes > 70).length,
    },
    {
      value: "recent",
      label: "Recent",
      count: mockPosts.filter(
        (p) => new Date(p.publishDate) > new Date("2024-01-10"),
      ).length,
    },
    {
      value: "featured",
      label: "Featured",
      count: mockPosts.filter((p) => p.featured).length,
    },
  ];
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-semibold">
              {selectedFilter === "all"
                ? "Latest Posts"
                : `${filterOptions.find((f) => f.value === selectedFilter)?.label} Posts`}
            </h2>
